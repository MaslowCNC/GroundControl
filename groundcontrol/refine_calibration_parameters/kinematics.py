...
class Calibration():
	def __init__( self ):
		self.l             = 310.0                               #horizontal distance between sled attach points
		self.s             = 139.0                               #vertical distance between sled attach points and bit

		self.h3            = 79.0                                #distance from bit to sled center of mass
		self.D             = 2978.4                              #distance between sprocket centers
		self.R             = 10.2                                #sprocket radius
		self.machineHeight = 1219.2                              #this is 4 feet in mm
		self.machineWidth  = 2438.4                              #this is 8 feet in mm
		self.motorOffsetY  = 463.0                               #vertical distance from the corner of the work area to the sprocket center
		self.chain1Offset  = 0                                   #number of links +,- that have slipped
		self.chain2Offset  = 0                                   #number of links +,- that have slipped

		self.rotationDiskRadius = 100
		self.chainSagCorrection = 0
		self.chainOverSprocket = 1
		self.isQuadKinematics    = True

		# these values were added to make calibration a bit easier and can
		#	be set to zero and ignored otherwise
		self.motorOffsetX = 0                                    #offset of the midpoint of the motors from center of work area in mm 
		self.rightMotorDeltaY = 0                                #offsetY of right motor relative to left motor
...
#    isQuadKinematics    = True
    ...
    def initializeValues( self ):
		...
		#utility variables
        self.DegPerRad = 360 / ( 4 * math.atan(1) )
        self.Time = 0
        self.Mirror = 0
		...
		# Motor axes length to the bit for triangular kinematics
        self.Motor1Distance = 0 #left motor axis distance to sled
        self.Motor2Distance = 0 #right motor axis distance to sled

		# output = chain lengths measured from 12 o'clock
        self.Chain1  = 0 #left chain length
        self.Chain2  = 0 #right chain length
...
#		self.i = 0
    ...
    def _verifyValidTarget(self, xTarget, yTarget):
        ...
        if (xTarget < -self.cal.machineWidth/2):
            xTarget = -self.cal.machineWidth/2

        elif (xTarget >  self.cal.machineWidth/2):
            xTarget =  self.cal.machineWidth/2

        elif (yTarget >  self.cal.machineHeight/2):
            yTarget =  self.cal.machineHeight/2
    ...
    def recomputeGeometry(self):
        ...
        '''
        self.h = math.sqrt((self.cal.l/2)*(self.cal.l/2) + self.cal.s * self.cal.s)
        self.Theta = math.atan(2*self.cal.s/self.cal.l)
        self.Psi1 = self.Theta - self.Phi
        self.Psi2 = self.Theta + self.Phi
        
		#	calculate motor1/2 x and y ignoring bed rotation
        self._xCoordOfMotor1 = self.cal.motorOffsetX - self.cal.D/2
        self._yCoordOfMotor1 = self.cal.motorOffsetY + self.cal.machineHeight/2

        self._xCoordOfMotor2 = self.cal.motorOffsetX + self.cal.D/2
        self._yCoordOfMotor2 = self.cal.motorOffsetY + self.cal.rightMotorDeltaY + self.cal.machineHeight/2
    ...
    def triangularInverse(self, xTarget, yTarget):
        ...
        Motor1Distance = math.sqrt(math.pow((self._xCoordOfMotor1 - xTarget),2)+math.pow((self._yCoordOfMotor1 - yTarget),2))
        Motor2Distance = math.sqrt(math.pow((self._xCoordOfMotor2 - xTarget),2)+math.pow((self._yCoordOfMotor2 - yTarget),2))

        #print( 'Motor1Distance = {:.03f}'.format( Motor1Distance ) )
        #print( 'Motor2Distance = {:.03f}'.format( Motor2Distance ) )
        ...
        if self.cal.chainOverSprocket == 1:
            Chain1Angle = math.asin((self._yCoordOfMotor1 - yTarget)/Motor1Distance) + math.asin(self.cal.R/Motor1Distance)
            Chain2Angle = math.asin((self._yCoordOfMotor2 - yTarget)/Motor2Distance) + math.asin(self.cal.R/Motor2Distance)

            Chain1AroundSprocket = self.cal.R * Chain1Angle
            Chain2AroundSprocket = self.cal.R * Chain2Angle
        else:
            Chain1Angle = math.asin((self._yCoordOfMotor1 - yTarget)/Motor1Distance) - math.asin(self.cal.R/Motor1Distance)
            Chain2Angle = math.asin((self._yCoordOfMotor2 - yTarget)/Motor2Distance) - math.asin(self.cal.R/Motor2Distance)

            Chain1AroundSprocket = self.cal.R * (3.14159 - Chain1Angle)
            Chain2AroundSprocket = self.cal.R * (3.14159 - Chain2Angle)

        #Calculate the straight chain length from the sprocket to the bit
        Chain1Straight = math.sqrt(math.pow(Motor1Distance,2)-math.pow(self.cal.R,2))
        Chain2Straight = math.sqrt(math.pow(Motor2Distance,2)-math.pow(self.cal.R,2))

        #Correct the straight chain lengths to account for chain sag
        Chain1Straight *= (1 + ((self.cal.chainSagCorrection / 1000000000000) * math.pow(math.cos(Chain1Angle),2) * math.pow(Chain1Straight,2) * math.pow((math.tan(Chain2Angle) * math.cos(Chain1Angle)) + math.sin(Chain1Angle),2)))
        Chain2Straight *= (1 + ((self.cal.chainSagCorrection / 1000000000000) * math.pow(math.cos(Chain2Angle),2) * math.pow(Chain2Straight,2) * math.pow((math.tan(Chain1Angle) * math.cos(Chain2Angle)) + math.sin(Chain2Angle),2)))
        ...
        Chain1 = Chain1 - self.cal.rotationDiskRadius
        Chain2 = Chain2 - self.cal.rotationDiskRadius
    ...
    def quadrilateralInverse(self, xTarget, yTarget):
        ...
        self.x = (self.cal.D/2.0) + xTarget
        self.y = (self.cal.machineHeight/2.0) + self.cal.motorOffsetY  - yTarget
        ...
        Tries = 0                                  #initialize
        if(self.x > self.cal.D/2.0):                              #the right half of the board mirrors the left half so all computations are done  using left half coordinates.
          self.x = self.cal.D-self.x                                  #Chain lengths are swapped at exit if the x,y is on the right half
          self.Mirror = True
        ...
        self.TanLambda = self.y/(self.cal.D-self.x)
        self.Y1Plus = self.cal.R * math.sqrt(1 + self.TanGamma * self.TanGamma)
        self.Y2Plus = self.cal.R * math.sqrt(1 + self.TanLambda * self.TanLambda)
        ...
        self.Crit[1] = - self._YOffsetEqn(self.Y1Plus, self.x - self.cal.h * self.CosPsi1, self.SinPsi1)
        self.Crit[2] = - self._YOffsetEqn(self.Y2Plus, self.cal.D - (self.x + self.cal.h * self.CosPsi2), self.SinPsi2)
        ...
        while (Tries <= self.MaxTries):
            ...
            self.Jac[3] = (self._YOffsetEqn(self.Y1Plus, self.x - self.cal.h * self.CosPsi1D, self.SinPsi1D) + self.Crit[1])/self.DeltaPhi
            self.Jac[4] = (self._YOffsetEqn(self.Y1Plus + self.DeltaY, self.x - self.cal.h * self.CosPsi1,self.SinPsi1) + self.Crit[1])/self.DeltaY
            self.Jac[5] = 0.0
            self.Jac[6] = (self._YOffsetEqn(self.Y2Plus, self.cal.D - (self.x + self.cal.h * self.CosPsi2D), self.SinPsi2D) + self.Crit[2])/self.DeltaPhi
            self.Jac[7] = 0.0
            self.Jac[8] = (self._YOffsetEqn(self.Y2Plus + self.DeltaY, self.cal.D - (self.x + self.cal.h * self.CosPsi2D), self.SinPsi2) + self.Crit[2])/self.DeltaY
            ...
            if (self.Y1Plus < self.cal.R):
                self.Y1Plus = self.cal.R

            self.Y2Plus = self.Y2Plus + self.Solution[2]                         #don't allow the anchor points to be inside a sprocke
            if (self.Y2Plus < self.cal.R):
                self.Y2Plus = self.cal.R
        ...
        self.Offsetx1 = self.cal.h * self.CosPsi1
        self.Offsetx2 = self.cal.h * self.CosPsi2
        self.Offsety1 = self.cal.h *  self.SinPsi1
        self.Offsety2 = self.cal.h * self.SinPsi2
        self.TanGamma = (self.y - self.Offsety1 + self.Y1Plus)/(self.x - self.Offsetx1)
        self.TanLambda = (self.y - self.Offsety2 + self.Y2Plus)/(self.cal.D -(self.x + self.Offsetx2))
        ...
        if(self.Mirror):
            Chain2 = math.sqrt((self.x - self.Offsetx1)*(self.x - self.Offsetx1) + (self.y + self.Y1Plus - self.Offsety1)*(self.y + self.Y1Plus - self.Offsety1)) - self.cal.R * self.TanGamma + self.cal.R * self.Gamma   #right chain length
            Chain1 = math.sqrt((self.cal.D - (self.x + self.Offsetx2))*(self.cal.D - (self.x + self.Offsetx2))+(self.y + self.Y2Plus - self.Offsety2)*(self.y + self.Y2Plus - self.Offsety2)) - self.cal.R * self.TanLambda + self.cal.R * self.Lambda   #left chain length
        else:
            Chain1 = math.sqrt((self.x - self.Offsetx1)*(self.x - self.Offsetx1) + (self.y + self.Y1Plus - self.Offsety1)*(self.y + self.Y1Plus - self.Offsety1)) - self.cal.R * self.TanGamma + self.cal.R * self.Gamma   #left chain length
            Chain2 = math.sqrt((self.cal.D - (self.x + self.Offsetx2))*(self.cal.D - (self.x + self.Offsetx2))+(self.y + self.Y2Plus - self.Offsety2)*(self.y + self.Y2Plus - self.Offsety2)) - self.cal.R * self.TanLambda + self.cal.R * self.Lambda   #right chain length
    ...
    def forward(self, chainALength, chainBLength):
        ...
        chainALength = chainALength + (self.cal.chain1Offset * self.cal.R)
        chainBLength = chainBLength + (self.cal.chain2Offset * self.cal.R)
    ...
    def _moment(self, Y1Plus, Y2Plus, Phi, MSinPhi, MSinPsi1, MCosPsi1, MSinPsi2, MCosPsi2):   #computes net moment about center of mass
        ...
        self.Offsetx1 = self.cal.h * MCosPsi1
        self.Offsetx2 = self.cal.h * MCosPsi2
        self.Offsety1 = self.cal.h * MSinPsi1
        self.Offsety2 = self.cal.h * MSinPsi2
        self.TanGamma = (self.y - self.Offsety1 + Y1Plus)/(self.x - self.Offsetx1)
        self.TanLambda = (self.y - self.Offsety2 + Y2Plus)/(self.cal.D -(self.x + self.Offsetx2))

        return self.cal.h3*MSinPhi + (self.cal.h/(self.TanLambda+self.TanGamma))*(MSinPsi2 - MSinPsi1 + (self.TanGamma*MCosPsi1 - self.TanLambda * MCosPsi2))
    ...
    def _YOffsetEqn(self, YPlus, Denominator, Psi):
        ...
        Temp = ((math.sqrt(YPlus * YPlus - self.cal.R * self.cal.R)/self.cal.R) - (self.y + YPlus - self.cal.h * math.sin(Psi))/Denominator)
        return Temp
