...
class BackgroundPickDlg(GridLayout, MakesmithInitFuncs):
    ...
    def __init__(self, data, **kwargs):
        ...
        # Create Texture (only needs to be done once)
        self.texture = Texture.create(size=(self.w, self.h))
        # ToDo: I don't know why I can't simply say self.Drawing here
        # but it explodes
    ...
    def redraw(self, *args):
        ...
        # and blit the buffer -- note that OpenCV format is BGR
        ...
        with self.Drawing.canvas:
            Rectangle(texture=self.texture, pos=self.Drawing.pos,
    ...
    def untouched(self, isntance, touch):
        ...
        # Get canvas zero and size, since touch comes back to us in absolute
        # screen coordinates
        ...
        self.texture.flip_vertical()
        cv2.circle(self.img, center, 20, (255, 255, 0), -1)  # Cyan
        if len(self.centers) < 4:
            self.redraw()
        else:
            # I was supposed to be working with a copy
            # why should I need to do this?!
            self.texture.flip_vertical()
            # All points picked, return to caller
            self.close(self)
